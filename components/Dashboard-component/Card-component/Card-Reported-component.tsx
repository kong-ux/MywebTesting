  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const processingData = DataCardReporting.find(item => item.state === "อยู่ละหว่างรับเรื่อง");
  ...
  return (
    <Card className="flex flex-col">
      <CardHeader className="">
        <CardTitle className="text-bold text-2xl">อยู่ละหว่างรับเรื่อง</CardTitle>
      ...
      <CardFooter className="flex-col gap-2 text-sm">
        <div className="flex items-center gap-2 font-medium leading-none">
          อยู่ละหว่างรับเรื่อง {chartData[0]?.num?.toLocaleString() || 0} รายการ จากทั้งหมด {totalReports.toLocaleString()} รายการ
        </div>
        <div className="leading-none text-muted-foreground">
          อยู่ละหว่างรับเรื่อง {percentageCompleted.toFixed(2)} % จากทั้งหมด 100%
        </div>
      </CardFooter>
    </Card>
  );
